  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={62}>
        ...
        {/* Messages */}
        <ScrollView ref={scrollRef} style={{ flex: 1 }} contentContainerStyle={{ padding: 14, gap: 12, paddingBottom: 20 }} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
      ...
      {/* Full AI Settings modal */}
      {showSettings && (
        <Modal visible transparent animationType="slide" onRequestClose={() => setShowSettings(false)}>
          <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.88)', justifyContent: 'flex-end' }} onPress={() => setShowSettings(false)}>
            <Pressable onPress={() => {}}>
              <View style={{ backgroundColor: T.surf, borderTopLeftRadius: 22, borderTopRightRadius: 22, maxHeight: '94%' }}>
                <View style={{ padding: 18, paddingBottom: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Text style={{ fontFamily: 'BarlowCondensed_900Black', fontSize: 20, color: T.txt }}>Настройки НЕЙРО</Text>
                  <TouchableOpacity onPress={() => setShowSettings(false)} style={{ width: 30, height: 30, borderRadius: 15, borderWidth: 1, borderColor: T.bord, backgroundColor: T.lo, alignItems: 'center', justifyContent: 'center' }}>
                ...
                <ScrollView contentContainerStyle={{ padding: 18, paddingTop: 14 }}>
                  {/* Provider grid */}
                  <Lbl T={T} style={{ marginBottom: 10 }}>Провайдер AI</Lbl>
                  {AI_PROVIDERS.map(p => {
                    const isActive = (aiConfig.provider || 'claude') === p.id;
                    return (
                      <TouchableOpacity key={p.id} onPress={() => setState(s => ({ ...s, aiConfig: { ...s.aiConfig, provider: p.id, model: p.defaultModel } }))}
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: 12, borderWidth: 1.5, borderColor: isActive ? p.color : T.bord, backgroundColor: isActive ? p.color + '15' : T.lo, marginBottom: 8 }}>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          {p.hint && !p.needsKey && <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 10, color: T.muted }}>{p.hint}</Text>}
                        </View>
                        {!p.needsKey && <View style={{ paddingHorizontal: 6, paddingVertical: 2, backgroundColor: T.success + '18', borderRadius: 6, borderWidth: 1, borderColor: T.success + '44' }}>
                          <Text style={{ fontFamily: 'BarlowCondensed_700Bold', fontSize: 10, color: T.success }}>FREE</Text>
                        </View>}
                        {isActive && <Check size={16} color={p.color} strokeWidth={2.5} />}
                      </TouchableOpacity>
                    );
                  })}
                  ...
                  {AI_PROVIDERS.find(p => p.id === (aiConfig.provider || 'claude'))?.needsKey && (
                    <View style={{ marginBottom: 16 }}>
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        ...
                        {AI_PROVIDERS.find(p => p.id === (aiConfig.provider || 'claude'))?.hint && (
                          <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 10, color: T.primary }}>
                            {AI_PROVIDERS.find(p => p.id === (aiConfig.provider || 'claude'))?.hint}
                          </Text>
                        )}
                      </View>
                      <TextInput
                        value={aiConfig.apiKey || ''} onChangeText={v => setState(s => ({ ...s, aiConfig: { ...s.aiConfig, apiKey: v } }))}
                        placeholder={`sk-... или ключ ${AI_PROVIDERS.find(p => p.id === aiConfig.provider)?.name}`}
                        placeholderTextColor={T.muted} secureTextEntry
                        style={{ height: 44, borderRadius: 10, borderWidth: 1.5, borderColor: aiConfig.apiKey ? T.success + '99' : T.bord, backgroundColor: T.lo, color: T.txt, fontFamily: 'Barlow_400Regular', fontSize: 14, paddingHorizontal: 12 }}
                      />
                      <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 11, color: T.warn, marginTop: 5 }}>⚠️ Хранится только на устройстве</Text>
                    </View>
                  )}
                  ...
                  {(aiConfig.provider === 'custom' || aiConfig.provider === 'ollama') && (
                    <View style={{ marginBottom: 16 }}>
                      ...
                      <TextInput
                        value={aiConfig.endpoint || ''} onChangeText={v => setState(s => ({ ...s, aiConfig: { ...s.aiConfig, endpoint: v } }))}
                        placeholder="https://api.example.com/v1/chat/completions"
                        placeholderTextColor={T.muted} autoCapitalize="none"
                        style={{ height: 44, borderRadius: 10, borderWidth: 1.5, borderColor: T.bord, backgroundColor: T.lo, color: T.txt, fontFamily: 'Barlow_400Regular', fontSize: 13, paddingHorizontal: 12 }}
                      />
                    </View>
                  )}
                  ...
                  {/* Model selector */}
                  {(() => {
                    const prov = AI_PROVIDERS.find(p => p.id === (aiConfig.provider || 'claude'));
                    ...
                    return (
                      <View style={{ marginBottom: 16 }}>
                        ...
                        <View style={{ gap: 6 }}>
                          {models.map((m: string) => {
                            const cur = (aiConfig.model || prov?.defaultModel) === m;
                            const isFast = m.includes('flash') || m.includes('mini') || m.includes('instant') || m.includes('haiku');
                            const isPro = m.includes('opus') || m.includes('pro') || m.includes('4o') || m.includes('70b');
                            return (
                              <TouchableOpacity key={m} onPress={() => setState(s => ({ ...s, aiConfig: { ...s.aiConfig, model: m } }))}
                                style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10, borderWidth: 1.5, borderColor: cur ? T.primary : T.bord, backgroundColor: cur ? T.primary + '15' : T.lo }}>
                                <View style={{ flex: 1 }}>
                                  <Text style={{ fontFamily: 'BarlowCondensed_700Bold', fontSize: 14, color: cur ? T.primary : T.txt }}>{m}</Text>
                                  <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 10, color: T.muted }}>
                                    {isFast ? '⚡ Быстрая и экономная' : isPro ? '🔬 Продвинутые задачи' : '⚖️ Баланс скорость/качество'}
                                    {m === prov?.defaultModel ? ' · по умолчанию' : ''}
                                  </Text>
                                </View>
                                {cur && <Check size={14} color={T.primary} strokeWidth={2.5} />}
                              </TouchableOpacity>
                            );
                          })}
                        </View>
                  ...
                  {/* Persona */}
                  <View style={{ marginBottom: 16 }}>
                    <Lbl T={T} style={{ marginBottom: 6 }}>Персонаж ИИ</Lbl>
                    {[
                      { id: '', label: 'Стандартный', desc: 'Профессиональный коуч', emoji: '🤖' },
                      { id: 'Ты строгий спартанский тренер. Говори кратко, по-военному. Только факты и команды.', label: 'Строгий', desc: 'Спартанский стиль', emoji: '⚔️' },
                      { id: 'Ты дружелюбный и воодушевляющий коуч. Используй позитивные эмодзи 💪🔥⭐', label: 'Мотиватор', desc: 'Позитивный стиль', emoji: '🔥' },
                      { id: 'Ты научный коуч. Объясняй через данные, исследования и физиологию.', label: 'Учёный', desc: 'Научный подход', emoji: '🔬' },
                    ].map(p => {
                      const cur = (aiConfig.persona || '') === p.id;
                      return (
                        <TouchableOpacity key={p.id} onPress={() => setState(s => ({ ...s, aiConfig: { ...s.aiConfig, persona: p.id } }))}
                          style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10, borderWidth: 1.5, borderColor: cur ? provColor : T.bord, backgroundColor: cur ? provColor + '15' : T.lo, marginBottom: 6 }}>
                          <Text style={{ fontSize: 18 }}>{p.emoji}</Text>
                          <View style={{ flex: 1 }}>
                            <Text style={{ fontFamily: 'BarlowCondensed_700Bold', fontSize: 14, color: cur ? provColor : T.txt }}>{p.label}</Text>
                            <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 11, color: T.muted }}>{p.desc}</Text>
                          </View>
                          {cur && <Check size={14} color={provColor} strokeWidth={2.5} />}
                        </TouchableOpacity>
                      );
                    })}
                  ...
                  {/* Context preview */}
                  <View style={{ padding: 12, backgroundColor: T.lo, borderRadius: 10, marginBottom: 16 }}>
                    <Text style={{ fontFamily: 'BarlowCondensed_700Bold', fontSize: 11, color: T.muted, letterSpacing: 1.5, marginBottom: 6 }}>ЧТО ВИДИТ ИИ В СИСТЕМНОМ ПРОМПТЕ</Text>
                    <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 12, color: T.muted, lineHeight: 17 }}>
                      • Всего тренировок · последние 5 тренировок с датой и сложностью{'\n'}
                      • Серия дней · личный рекорд{'\n'}
                      • Активные цели с прогрессом{'\n'}
                      • Настроение за последние 7 дней{'\n'}
                      • Персонаж / дополнительные инструкции
                    </Text>
                  </View>

                  <TouchableOpacity onPress={() => setShowSettings(false)} style={{ height: 46, borderRadius: 12, backgroundColor: T.primary, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ fontFamily: 'BarlowCondensed_900Black', fontSize: 16, color: '#000' }}>Сохранить настройки</Text>
                  </TouchableOpacity>
                </ScrollView>
              </View>
            </Pressable>
