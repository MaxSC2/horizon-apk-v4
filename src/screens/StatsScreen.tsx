        ...
        {sub === 'profile' && (
          <>
            ...
            <TouchableOpacity onPress={() => Alert.alert('Сбросить всё?', 'Тренировки, задачи, цели, дневник — всё удалится.', [{ text: 'Отмена', style: 'cancel' }, { text: 'Удалить всё', style: 'destructive', onPress: () => setState({ ...DEFAULTS, themeId: state.themeId }) }])}
              style={{ height: 48, borderRadius: 12, borderWidth: 1.5, borderColor: T.danger + '66', backgroundColor: T.danger + '15', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginBottom: 12 }}>
            ...
            {/* v4.1 — Export data to JSON (uses Sharing for save / send) */}
            <TouchableOpacity
              onPress={async () => {
                try {
                  ...
                  if (await Sharing.isAvailableAsync()) {
                    ...
                    Alert.alert('✓ Готово', `Данные экспортированы (${json.length} символов). Sharing недоступен — файл сохранён в кэш.`);
                  }
                } catch (e: any) {
                  Alert.alert('Ошибка экспорта', e?.message || String(e));
                }
              }}
              style={{ height: 48, borderRadius: 12, borderWidth: 1.5, borderColor: T.primary + '66', backgroundColor: T.primary + '15', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginBottom: 12 }}
            >
              <Text style={{ fontFamily: 'Barlow_400Regular', fontSize: 16, color: T.primary }}>⬇</Text>
